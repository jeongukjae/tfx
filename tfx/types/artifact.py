...
def _decode_struct_value(
  ...
  if struct_value is None:
    return None
  decoded_value = json_format.MessageToDict(struct_value)
  if _JSON_SINGLE_VALUE_KEY in decoded_value and len(decoded_value) == 1:
    return decoded_value[_JSON_SINGLE_VALUE_KEY]
  else:
...
def _encode_struct_value(value: JsonValueType) -> Optional[struct_pb2.Struct]:
  ...
  json_format.ParseDict(value, struct_value)
  return struct_value
